...
def register():
    if request.method == 'POST':
        ...
        try:
            # VULNERABILITATE: Parola se salveaza direct in clar
...
def login():
    if request.method == 'POST':
        ...
        # VULNERABILITATE: User Enumeration 
        if user is None:
...
# --- 3.4 Resetare parola (Vulnerabil: Predictibil) ---
@app.route('/forgot-password', methods=['GET', 'POST'])
def forgot_password():
    if request.method == 'POST':
        email = request.form['email']
        # Generam link-ul nesigur (vulnerabilitatea 4.6)
        link_resetare = url_for('reset_password', user=email, _external=True)
        return f"Link-ul de resetare a fost generat: <a href='{link_resetare}'>{link_resetare}</a>"
...
def reset_password():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        # Vulnerabil: Schimbam parola doar pe baza email-ului din URL
    ...
    return render_template('reset_password.html', email=user_email)
